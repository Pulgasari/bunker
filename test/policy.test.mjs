...
{
  const cache = createPolicy({ staleTtl: 60, ttl: 30 });
  await cache.set('k', 'v');
...
// :::::: swr: fresh serves cache, miss awaits, stale serves old and revalidates
{
  ...
  const cache   = createPolicy({ staleTtl: 500, ttl: 30 });
...
{
  let calls = 0;
  const cache   = createPolicy({ staleTtl: 500, ttl: 20 });
...
{
  const errors  = [];
  const failing = createPolicy({ onError: e => errors.push(e.operation), staleTtl: 500, ttl: 20 });
...
// :::::: `max` is an l1 ceiling and must not delete from l2
{
  ...
  const cache  = createPolicy({ driver, max: 3 });
...
{
  const driver = createMemoryDriver();
  const cache  = createPolicy({ driver, maxEntries: 2 });
...
{
  const memoryOnly = createPolicy({ ttl: 20 });
  ...
  const driver = createMemoryDriver();
  const cache  = createPolicy({ driver, ttl: 20 });
...
{
  const driver = createMemoryDriver();
  const first  = createPolicy({ driver, namespace: 'aufbau' });
  await first.set('theme', 'oled');

  const second = createPolicy({ driver, namespace: 'aufbau' });
  assert.equal(await second.get('theme'), 'oled', 'a cold l1 must fall through to l2');

  const other = createPolicy({ driver, namespace: 'docs' });
...
// :::::: a failing l2 write does not reject the caller
{
  ...
  const errors = [];
  const cache  = createPolicy({ driver: broken, onError: e => errors.push(e.operation) });
...
// :::::: garbage in l2 reads as a miss rather than blowing up
{
  ...
  const cache = createPolicy({ driver });
