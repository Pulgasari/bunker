...
import type { Files } from '@bunker/files';
import type { Storage } from '@bunker/storage';

export interface BunkerOptions {
  /** L1 entry ceiling for the cache. */
  max?: number;
  /** L2 entry ceiling for the cache, enforced by `cache.prune()`. */
  maxEntries?: number;
  ...
  staleTtl?: number;
  /** IndexedDB table backing the cache. Defaults to `kv`. */
  table?: string;
  ttl?: number | null;
  version?: number;
}
...
export { createFiles } from '@bunker/files';
